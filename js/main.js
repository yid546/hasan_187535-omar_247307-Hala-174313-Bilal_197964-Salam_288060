...
document.addEventListener('DOMContentLoaded', function() {
    const toggle = document.getElementById('modeLangToggle');
    const htmlElement = document.documentElement;
    const toggleLabel = document.getElementById('toggleLabel');

    // 1. تحديد الحالة الأولية من Local Storage (أو الافتراضية)
    const savedTheme = localStorage.getItem('theme') || 'light';
    const savedLang = localStorage.getItem('lang') || 'ar';
    
    // تطبيق السمة واللغة المحفوظتين عند تحميل الصفحة
    htmlElement.setAttribute('data-bs-theme', savedTheme);
    htmlElement.setAttribute('lang', savedLang);
    htmlElement.setAttribute('dir', savedLang === 'ar' ? 'rtl' : 'ltr');

    // تحديث حالة زر التبديل (Checked = Dark/EN)
    if (savedTheme === 'dark' || savedLang === 'en') {
        toggle.checked = true;
    }

    function updateToggleLabel(theme, lang) {
        let modeIcon = theme === 'dark' ? '<i class="fas fa-sun me-1"></i>' : '<i class="fas fa-moon me-1"></i>';
        let langText = lang === 'ar' ? 'AR' : 'EN';
        toggleLabel.innerHTML = `${modeIcon}/${langText}`;
    }

    // تحديث الـ Label عند التحميل الأولي
    updateToggleLabel(savedTheme, savedLang);


    // 2. معالج حدث التبديل (Toggle Event Handler)
    toggle.addEventListener('change', function() {
        
        // إذا كان الزر مُفعلاً، فسننتقل إلى الوضع الداكن واللغة الإنجليزية
        if (this.checked) {
            var newTheme = 'dark';
            var newLang = 'en';
        } else {
            // إذا كان غير مُفعلاً، ننتقل إلى الوضع الفاتح واللغة العربية
            var newTheme = 'light';
            var newLang = 'ar';
        }
        
        // تطبيق وتبديل الوضع الداكن
        ...
        // تحديث الـ Label
        updateToggleLabel(newTheme, newLang);
    });
});